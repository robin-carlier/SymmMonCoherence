...
TODO: module systemize part of this file
-/
...
theorem exchangeProperty_iff_deletionProperty : S.DeletionProperty ↔ S.ExchangeProperty where
  ...
  mpr E := by
    ...
    have eq₁ := List.eraseIdx_append_of_length_le
      (k := j + (ω.take i).length) (l := ω.take i) (by lia) (ω.drop (i + 1))
...
private lemma even_length_of_hom_wordProd_eq_one
  ...
  · obtain ⟨i, j, hij, hj, e⟩ := D ω hred
    ...
    have := ih k (by lia) ((ω.eraseIdx i).eraseIdx (j - 1)) (by grind) (by grind)
...
private lemma fine_non_maximal_index
  ...
  · -- easy case: the heads of the list cancel, giving a shorter relation
    ...
    apply ih ω.length (by lia) ω ω' rfl hl
      (by
...
private lemma reduction_step
  ...
  · exact fun h => S.nonReduced_case E (by grind) h (by simpa [hn] using ih) hred
  push_neg at hred
  intro h
  ...
  · suffices h₀ : S.Fine (s₁' :: (s₁ :: ω).dropLast) (s₁ :: ω) by
      ...
      have := ih _ (by simp; lia) _ _ rfl (by simp; lia) hi'
      grind [CoxeterSystem.wordProd_cons]
    assumption
  · apply ih₂ (i := i) <;> grind
...
private lemma fine_alternatingWord (s₁ s₁' : B) (j : ℕ) :
  ...
  simp only [CoxeterSystem.prod_alternatingWord_eq_mul_pow, CoxeterMatrix.toCoxeterSystem_simple,
    ite_mul, one_mul, wordProd_append, wordProd_singleton] at h ⊢
  ...
  · have h₁ : Even (j + 1) := by simp [hj, parity_simps]
    ...
    simp only [h₁, ↓reduceIte, h₂, map_pow, map_mul, hj] at h ⊢
    rw [← mul_inv_eq_one] at h ⊢
...
private lemma fine_alternatingWord_append_induction
  ...
  | succ k hrec =>
      -- We need some lemmas to help the lia calls
    have min₁ : min k ω.length = k := by rw [min_eq_left_iff]; lia
    have min₁' : min (k + 1) ω.length = (k + 1) := by rw [min_eq_left_iff]; lia
    cases j with
    ...
    | succ j =>
      ...
      -- need some lemmas to help the lia calls
      ...
      | succ k => -- this case reduces to the induction hypothesis.
        ...
        suffices h₀ :
          ...
          rw [h₀.1, h₀.2] at h ⊢
          exact hrec (s₁ := s₁) (s₁' := s₁') (ω := ω) (j + 2) (by lia)
            (by simp only [Order.lt_add_one_iff] at ih; grind) (by grind) h
        dsimp [s₀, s₀', ω₀, ω₀']
...
lemma eq_of_hom_eq_one_of_exchangeProperty' (E : S.ExchangeProperty) (ω ω' : List B)
  ...
  | succ n =>
    ...
    · -- Case 2 : the words are reduced: this is the harder case.
      ...
      cases ω' with | nil => grind | cons s₁' ω' =>
      exact S.sub_induction_1 E rfl (by grind) (by grind)
        (fun _ _ ω ω' _ _ _ h i hi hi' =>
          S.fine_non_maximal_index (by grind) (by grind) _ hi hi' (by grind) (by grind)) h
...
theorem injective_hom_of_exchangeProperty (E : S.ExchangeProperty) : Function.Injective S.hom := by
  ...
  | succ k'' hr =>
    ...
    intro _
    exact S.eq_of_hom_eq_one_of_exchangeProperty' E _ _ (by lia) (by assumption)
