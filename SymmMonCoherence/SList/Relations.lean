...
The idea is simple: the relations that defines symmetric lists are essentially in
one-to-one correspondence with the relations that presents
...
permutation of the indices of the source and targed that this morphism performed.
...
1. Any two formal composition of generating morphisms with same source, target and label are equal
2. Any two formal composition of generating morphisms with same source and label have equal target
3. That for suitable labels and source, one can find formal composition of generating morphisms out
of the given source with given label.
4. Using 1 and 2, that for each relation in the Coxeter matrix `Ainf`, one can find an
explicit morphisms whose label realize these relations, and that these morphisms reduce to
identities in `SList`.
5. Using all of the points above and the definiton of the equivalence relations generated by the
...
one needs to decompose the source and target of the morphim, construct by hand a "candidate"
...
invoking the relations that defines symmetric lists.
...
two words may *a priori* be related because of transitivity through a word that are "too big".
...
`FreeSListQuiv C` whose label correspond to a relation in `Ainf.Monoid` i.e whose label is one of
- `«i» * «i»`
- `«i» * «i + 1» * «i» * «i + 1» * «i» * «i + 1»`
- `«i + 1» * «i» * «i + 1» * «i» * «i + 1» * «i»`
- `«i» * «j» * «i» * «j»` for `|j - i| ≥ 2`
...
one must first "decompose" L in a way that will make naming explicit out of `L` possible,
then one introduces a "candidate" morphism that realizes almost tautologicaly the given label, this
...
hand separately, but are fortunately not too difficult: all these paths types are subsingletons! -/
